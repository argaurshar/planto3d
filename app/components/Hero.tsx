...
/**
 * A real Voxa render used as the payoff frame of the demo. Served from
 * `public/`, so the Pages build needs the basePath prefix. Only the bedroom has
 * a real render; other rooms fall back to the stylised CSS interior, and so does
 * this one if the file is missing — the demo never shows a broken image.
 */
const RENDER_ROOM_ID = "bedroom";
const BASE_PATH = process.env.NEXT_PUBLIC_BASE_PATH || "";
const RENDER_SRC = `${BASE_PATH}/hero-render.jpg`;
...
export default function Hero({ onPlanSelected }: Props) {
  ...
  const showPhoto = focus.id === RENDER_ROOM_ID && !renderMissing;
  const is3d = stage === 1;
            ...
            >
              {showPhoto ? (
                <div className="relative h-full w-full">
                  {/* Only fetched once the demo is near the render stage, so the
                      landing paint doesn't pay for it. */}
                  {stage >= 2 && (
                    /* eslint-disable-next-line @next/next/no-img-element */
                    <img
                      src={RENDER_SRC}
                      alt="Photorealistic eye-level render of the bedroom generated by Voxa"
                      className="h-full w-full object-cover"
                      decoding="async"
                      onError={() => setRenderMissing(true)}
                    />
                  )}
                  <RenderOverlay roomName={focus.name} animate={rendering} />
                </div>
              ) : (
...
function InteriorMock({ roomName, animate }: { roomName: string; animate: boolean }) {
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      <RenderOverlay roomName={roomName} animate={animate} />
    </div>
  );
}

/** Room label + Verified badge, shared by the real render and the CSS mock. */
function RenderOverlay({ roomName, animate }: { roomName: string; animate: boolean }) {
  return (
    <>
